...
{
  __LLANG_NODE *node;
  for(node = ll_list->tail; node != NULL; node = node->prev)
    __llang_list_delete(ll_list, node);	
}
...
llang_i32 __llang_list_indexof(__LLANG_LINKED_LIST *ll_list, __LLANG_NODE *ll_node)
{
  llang_i32 index = 0;
  __LLANG_NODE *_node;
  for(_node = ll_list->head; _node != NULL; _node = _node->next, index++)
    if(_node==ll_node)
      return index;
  return -1;
}
		...
		*data_ptr, llang_lambda cmp_proc)
{
  ...
  llang_i32 i = 0;
  __LLANG_NODE *node;
  for(node = ll_list->head ; node != NULL ; node = node->next, i++)
  {
  ...
  llang_bool __is_sorted(ll_list, data_cmp_proc)
  __LLANG_LINKED_LIST *ll_list;
  llang_lambda data_cmp_proc;
  {
    __LLANG_NODE *node;
    if(ll_list->length < 2) 
      return true;
    for(node = ll_list->head; node->next != NULL; node = node->next)
    {
  ...
  void __insertion_sort(ll_list, data_cmp_proc)
  __LLANG_LINKED_LIST *ll_list;
  llang_lambda data_cmp_proc;
  { 
    if(ll_list->length < 2) return;
    __LLANG_NODE *ll_node;
    for(ll_node = ll_list->head->next; ll_node ; ll_node = ll_node->next)
    {
  ...
  void __quicksort(ll_list, ll_head, ll_tail, data_cmp_proc)
  __LLANG_LINKED_LIST *ll_list;
  __LLANG_NODE *ll_head;
  __LLANG_NODE *ll_tail;
  llang_lambda data_cmp_proc;
  { 
    if((ll_head == ll_tail) || !ll_head || !ll_tail) 
      return;
    ...
    if(ll_head != ll_prt_node)
      __quicksort(ll_list, ll_head, ll_prt_node->prev, data_cmp_proc);
    if(ll_tail != ll_prt_node)
      __quicksort(ll_list, ll_prt_node->next, ll_tail, data_cmp_proc);	
  };
...
void __llang_list_sort(__LLANG_LINKED_LIST *ll_list, llang_lambda cmp_proc)
{
  ...
  if((ll_list->length < 2) || __is_sorted(ll_list, data_cmp_proc)) 	
    return;
    
  if(ll_list->length <= 25)
    __insertion_sort(ll_list, data_cmp_proc);
  else
    __quicksort(ll_list, ll_list->head, ll_list->tail, data_cmp_proc);
};
...
llang_u32 __llang_list_remove_duplicates(__LLANG_LINKED_LIST *ll_list, llang_lambda cmp_proc)
{
  ...
  __LLANG_NODE *ll_node;
  for(ll_node = ll_list->tail; ll_node->prev != NULL; ll_node = ll_node->prev)
  {
