...
LLANG_TOKEN *__token_next(LLANG_LEXER *lexer)
{
  ...
  for (;;) {
	...
	if(type==TT_NUMBER) {
	  if(isalpha(next) && tolower(next) != 'e') {
	    ll_lexer_error("Invalid number");
	  }
	  else if((ispunct(next) && next != '.') || isspace(next)) 
	    goto __done;
	} else if(type==TT_HEXNUM) {
	  if(!isxdigit(next)) {
	    ll_lexer_error("Invalid Hex Character!");
	  }
	  else if(ispunct(next) || isspace(next)) 
	    goto __done;
	} else if(type==TT_IDENT) {
	  if(isspace(next) || (ispunct(next) && next != '_')) 
	    goto __done;		
	}
	break;
      case '_':
	...
	if(type==TT_NUMBER) {
	  if(tolower(curr=='e')) {
	    if((isdigit(next)==false) && (next != '+') && (next != '-')) 
	    {
	      ll_lexer_error("Invalid number")}; 
	      if(eps>1) {
	        ll_lexer_error("invalid number too many 'e'");
	      };
	      eps++;
	  };
	} else if(type==TT_IDENT) { 
	...
	goto __done;
      ...
      case '-':
	if(type == TT_NUMBER) { 
	  if(tolower(prev) == 'e' && !isdigit(next)) {
	    ll_lexer_error("invalid number");
	  }
	} else {
	  type = TT_MINUS;
...
__done:	
  ...
  if(type== TT_NONE) 
    return NULL;
  if(type==TT_NUMBER && (lex_pos-i) > 25) {
    ll_lexer_error("invalid width, number is too long");
  }
  else if(type==TT_IDENT) { 
    if(lex_pos-i >= 256) 
      ll_lexer_error("invalid identifier, Too long"); 
  }
	...
	{	
	  ...
	  if(isdigit(buf[lex_pos+1])) {
	    b <<= 4;
	    b = b | (isdigit(buf[++lex_pos]) ? buf[lex_pos] - '0' : tolower(buf[lex_pos]) - 'a');
	  }
          llang_queue_put(q, (llang_ptr) &b);
	}
	else {
	  ll_lexer_error("invalid hex char after \\x");
	};
        bf = false;
      }
    ...
    if((type==TT_QUOTE&&buf[lex_pos] != '\'') || (type==TT_DQUOTE&&buf[lex_pos]!='"')) 
      ll_lexer_error("invalid string");
  ...
  if(type== TT_STRING) { 
    int n = 0; 
    while(!llang_queue_empty(q)) {
      s[n++] = * (llang_char *) llang_queue_get(q);
    }
  }
  else {
    int j;
